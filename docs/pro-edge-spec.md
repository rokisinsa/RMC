...
状態：手順2の設計への追加として、データ構造・スキーマ・計算エンジン・検証・テストを作成済みです。画面（手順3）はまだ実装していません。`data/pro_edge.json` は空の下書きで、架空のサンプルはテスト用フィクスチャにだけあります。
...
- **独自の識別子**：各カードは、自分の系統の `system`・`run_id`・`discovered_at`・`analysis_id` を持ちます。

  | 系統 | カードID | run_id | analysis_id |
  |---|---|---|---|
  | ① 推奨取引 | `rec-` | `rec-` | `rec-an-` |
  | ② VALUE① | `v1-` | `v1-` | `v1-an-` |
  | ③ VALUE② | `v2-` | `v2-` | `v2-an-` |
  | ④ PRO EDGE | `pe-` | `pe-` | `pe-an-` |

- **流用の禁止**：他系統の探索回・分析ID・カードIDを参照すると error です。新規カードは分析IDが必須で、移行用の探索回は使えません。
