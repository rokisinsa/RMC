...
export const SYSTEMS = ["recommendations", "experience", "value1", "value2"];
export const DISCOVERY_SYSTEMS = ["recommendations", "value1", "value2"];
export const ID_PREFIX = { recommendations: "rec-", experience: "exp-", value1: "v1-", value2: "v2-" };
...
export const LOCKED_FIELDS = [
  ...
  "discovered_at", "run_id", "locked_at", "locked", "live",
];
...
// ── 3分析系統の独立性 ────────────────────────────────────
// datasets: { matches?, recommendations?, experience?, value1?, value2? }
export function checkIndependence(datasets) {
  ...
  for (const system of present) {
    const data = datasets[system];
    for (const id of [...data.picks.map(p => p.id), ...(data.discovery_runs ?? []).map(r => r.run_id)]) {
        ...
        for (const pb of b.picks) {
          ...
          if (sameSummary || sameRange) {
            out.push(issue("warning", "POSSIBLY_SHARED_ANALYSIS", `${a.system}/${b.system}`, `${pa.id},${pb.id}`,
              `同じ試合・選択で事前分析が一致（${sameSummary ? "分析文" : "推定勝率レンジ"}）。系統ごとに独立して分析したか確認`));
          }
        }
      }
    }
...
export function checkTimes(systemData, matchesById) {
  ...
  for (const pick of systemData.picks) {
    ...
    const start = matchesById.get(pick.match_id)?.start_at ?? null;
    ...
    if (start != null) {
      ...
      if (!pick.live) {
        ...
        out.push(issue("error", "TIME_LIVE_WITHOUT_BET_AT", sys, id, "ライブ取引は bet_at が必須"));
      }
    }
...
export function checkOdds(systemData) {
  ...
  for (const pick of systemData.picks) {
    ...
    if (pick.odds_taken != null && pick.bet_at == null && pick.locked_at == null) {
      out.push(issue("error", "ODDS_TAKEN_WITHOUT_TIME", sys, id, "odds_taken には取得時点（bet_at か locked_at）が必要"));
    }
  }
  return out;
}
...
export function checkConditions(systemData, matchesById) {
  ...
  for (const pick of systemData.picks) {
    ...
    const start = matchesById.get(pick.match_id)?.start_at;
...
export function checkLockedImmutable(prevData, nextData) {
  ...
  for (const before of prevData.picks) {
    if (before.locked_at == null) continue;
    const after = next.get(before.id);
    if (!after) {
      out.push(issue("error", "LOCKED_PICK_DELETED", sys, before.id, "locked 済みのカードが削除された"));
      continue;
    }
    for (const field of LOCKED_FIELDS) {
      if (!deepEqual(before[field] ?? null, after[field] ?? null)) {
        out.push(issue("error", "LOCKED_FIELD_CHANGED", sys, before.id,
          `locked 後に ${field} が変更された（${JSON.stringify(before[field])} → ${JSON.stringify(after[field])}）。再評価は reevaluations へ追記する`));
      }
    }
    ...
    const prevRe = before.reevaluations ?? [], nextRe = after.reevaluations ?? [];
    if (nextRe.length < prevRe.length || prevRe.some((r, i) => !deepEqual(r, nextRe[i]))) {
      out.push(issue("error", "REEVALUATION_REWRITTEN", sys, before.id, "reevaluations は追記のみ（既存の再評価を変更・削除しない）"));
    }
