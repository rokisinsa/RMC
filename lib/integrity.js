...
const ESTIMATE_KEYS = ["prob", "prob_lo", "prob_hi", "prob_point", "ev_lo", "ev_hi", "market_gap_lo", "market_gap_hi"];
  ...
  for (const m of matchesData.matches) {
    ...
    if (recorded !== (m.start_at != null)) {
      out.push(issue("error", "START_STATUS_MISMATCH", "matches", m.id,
        "start_at は start_time_status=recorded のときだけ持つ（未確認・不明なら null）"));
    }
    ...
    for (const [period, side] of Object.entries(r?.winners ?? {})) {
      ...
      if (score) {
        ...
        if (actual !== side) {
          out.push(issue("error", "WINNER_SCORE_CONFLICT", "matches", m.id, `${period} の勝者 ${side} がスコア ${score.home}-${score.away} と矛盾`));
        }
      }
    }
  }
