...
// 分析ID（analysis_id）は ④ PRO EDGE だけの追跡項目。①推奨・②VALUE①・③VALUE②には適用しない（④追加で①②③を変更しない）
export const ANALYSIS_ID_PREFIX = { pro_edge: "pe-an-" };
  ...
  for (const system of present) {
    ...
    for (const pick of data.picks) {
      ...
      if (DISCOVERY_SYSTEMS.includes(system)) {
        ...
        // ④ PRO EDGE のカードは④自身の分析IDが必須（①②③には適用しない）
        if (system === "pro_edge") {
...
export function checkLockedImmutable(prevData, nextData) {
  ...
  for (const before of prevData.picks) {
    ...
    // ④：分析IDは変更不可、購入価格は一度入れたら変更不可、価格スナップショットは追記のみ（①②③のカードはこれらの項目を持たない）
    if (sys === "pro_edge" && !deepEqual(before.analysis_id ?? null, after.analysis_id ?? null)) {
      out.push(issue("error", "LOCKED_FIELD_CHANGED", sys, before.id, "analysis_id が変更された"));
    }
    if (sys === "pro_edge" && before.bet_odds != null && !deepEqual(before.bet_odds, after.bet_odds)) {
      out.push(issue("error", "LOCKED_FIELD_CHANGED", sys, before.id, "bet_odds が変更された"));
    }
    if (sys === "pro_edge" && Array.isArray(before.price_snapshots)) {
