...
function compoundCard(comp, audit) {
  if (!comp) return "";
  const unconfirmed = audit?.status === "order_unconfirmed";
  const alt = audit?.alternative_by_registration;
  const note = `試合開始時刻順：現在資金 ${amount(comp.balance)}／ストック ${amount(comp.stock)}（確保 ${comp.cycles}回・失敗 ${comp.failures}回）`
    + (alt ? `<br>旧画面と同じ登録時刻順：現在資金 ${amount(alt.balance)}／ストック ${amount(alt.stock)}` : "")
    + (unconfirmed ? `<br>${esc(audit.reasons.join("／"))}` : "");
  return card(unconfirmed ? "複利（参考値・順序未確定）" : "複利", money(comp.profit), { tone: unconfirmed ? "muted" : toneOf(comp.profit), note });
}
...
function renderValue(v, { id, title, note, second }) {
  ...
  return `<section class="system value" id="${id}"><h2 class="section-title">${esc(title)}</h2><p class="note">${esc(note)}</p>
    <h3 class="panel-title">本成績</h3><div class="summary">${officialCards(v.official, v.official_compound, second
      ...
      : [card("1/4ケリー", "集計保留", { note: "推定勝率がレンジ（点推定なし）のため算出しない" })], v.compound_audit)}</div>
