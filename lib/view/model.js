...
function systemRows(data, matchesById, summary, ctx, legacyAnalysis = null) {
  ...
  for (const r of ctx.postMatch?.[data.system]?.reviews ?? []) reviewsById.set(r.pick_id, [...(reviewsById.get(r.pick_id) ?? []), r]);
  const byId = new Map();
  ...
  return data.picks.map(p => {
    ...
    row.post_match_reviews = reviewsById.get(p.id) ?? [];
    return row;
  });
}
...
// 複利は取引の順序で結果が変わる。正式値として採用する順序規則（docs/gpt-update-spec.md「複利の順序規則」）：
//   1) 確定した全取引に投入時刻 bet_at があり、その試合に結果確定時刻 result_confirmed_at がある
//   2) bet_at の昇順に並べ、各取引の bet_at が直前の取引の result_confirmed_at 以降（前の結果が確定してから次を投入）
//   3) bet_at が同時刻の取引が無い
// 1〜3 をすべて満たすときだけ bet_at 順の複利を正式値とする。満たさない場合は「参考値・取引順序未確定」とし、
// 試合開始時刻順と旧画面の登録時刻順の2通りを参考として示す（過去分の順序を推測しない）。
export function compoundAudit(officialGroups) {
  ...
  const base = { alternative_by_registration: compound(byRegistration, 100), official_order: null };
  if (!settled.length) return { status: "no_settled", reasons: [], ...base };
  const reasons = [];
  const noBet = settled.filter(g => g.pick.bet_at == null);
  const noConfirm = settled.filter(g => g.match?.result_confirmed_at == null);
  if (noBet.length) reasons.push(`投入時刻（bet_at）の記録がない取引 ${noBet.length}件`);
  if (noConfirm.length) reasons.push(`結果確定時刻（result_confirmed_at）の記録がない取引 ${noConfirm.length}件`);
  if (!reasons.length) {
    const ordered = [...settled].sort((a, b) => Date.parse(a.pick.bet_at) - Date.parse(b.pick.bet_at));
    for (let i = 1; i < ordered.length; i++) {
      const prev = ordered[i - 1], next = ordered[i];
      if (Date.parse(next.pick.bet_at) === Date.parse(prev.pick.bet_at)) reasons.push(`${prev.pick.id} と ${next.pick.id} の投入時刻が同じ`);
      else if (Date.parse(next.pick.bet_at) < Date.parse(prev.match.result_confirmed_at)) reasons.push(`${next.pick.id} は直前の ${prev.pick.id} の結果確定前に投入`);
    }
    if (!reasons.length) {
      return { status: "confirmed", reasons, ...base, official_order: compound(ordered.map(g => ({ ...g.settlement, orderKey: g.pick.bet_at })), 100) };
    }
  }
  return { status: "order_unconfirmed", reasons, ...base };
}
...
export function buildViewModel(ds, { proEdgeConfig = {}, now = new Date().toISOString() } = {}) {
  ...
  const ctx = {
    now, reviews: pendingReviews(ds.match_updates),
    postMatch: Object.fromEntries(["recommendations", "experience", "value1", "value2", "pro_edge"].map(s => [s, ds[`post_match_${s}`] ?? null])),
  };
