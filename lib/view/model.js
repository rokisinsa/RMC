...
function quality(pick, match, extra = []) {
  const q = [];
  ...
  } else if (match?.status === "scheduled") q.push({ key: "result", level: "info", label: "試合前" });
...
function baseRow(pick, match, bucket, settlement) {
  return {
    id: pick.id,
    system: pick.system,
    bucket,
...
function systemRows(data, matchesById, summary) {
  const byId = new Map();
  ...
  return data.picks.map(p => {
    ...
    return baseRow(p, matchesById.get(p.match_id), bucket, settlement);
  });
}
...
export function buildViewModel(ds, { proEdgeConfig = {} } = {}) {
  const matchesById = new Map(ds.matches.matches.map(m => [m.id, m]));

  // ① 推奨取引
  const recSummary = summarizeSystem(ds.recommendations, matchesById);
  const recRows = recOrder(systemRows(ds.recommendations, matchesById, recSummary));
  ...
  const experience = { summary: expSummary.official, rows: recOrder(systemRows(ds.experience, matchesById, expSummary)) };

  // ② VALUE① / ③ VALUE②（本成績と参考成績は別集計）
  const valueView = data => {
    const s = summarizeSystem(data, matchesById);
    const rows = legacyOrder(systemRows(data, matchesById, s));
    return {
      official: s.official,
  ...
  const value1 = valueView(ds.value1);
  const value2 = valueView(ds.value2);
  ...
  for (const [bucket, list] of Object.entries(pe.groups)) {
    for (const g of list) {
      ...
      const row = baseRow(g.pick, match, bucket, g.settlement);
  ...
  return {
    meta: {
      ...
      update_runs: ds.matches.update_runs ?? [],
    },
