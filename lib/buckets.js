...
// ④ PRO EDGE
//   accepted → official / watch → reference / rejected → excluded
// 推奨取引・経験値取引
//   すべて official
...
export function classifyPick(pick, match) {
  ...
  switch (pick.system) {
    ...
    case "value1":
      ...
      return "unlocked";
    case "pro_edge": {
      const decision = pick.locked?.decision ?? null;
      if (decision === "accepted") return "official";
      if (decision === "watch") return "reference";
...
export function summarizeSystem(systemData, matchesById) {
  ...
  const verdictCounts = {};
  for (const pick of systemData.picks) {
    const v = pick.locked?.verdict ?? pick.locked?.decision ?? "(none)";
