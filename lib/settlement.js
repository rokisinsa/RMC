...
export function outcomeFromMatch(pick, match) {
  ...
  if (!score) return { state: "pending", reason: `${period} のスコアが未入力` };
  ...
  return { state: winner === pick.selection ? "win" : "loss", reason: `${period} ${score.home}-${score.away}` };
}
...
export function settlePick(pick, match, { notionalStake = null } = {}) {
  ...
  const odds = pick.odds_taken ?? null;
  ...
  return {
    ...
    orderKey: match?.start_at ?? pick.bet_at ?? pick.locked_at ?? pick.discovered_at ?? null,
  };
}
