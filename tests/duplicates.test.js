...
function withDuplicate(flags = []) {
  ...
  const copy = structuredClone(ds.recommendations.picks[0]);   // m1 / match_1x2 / home
  copy.id = "rec-r1-dup";
  copy.flags = flags;
...
test("同一系統内の 同一試合＋同一市場＋同一選択 を重複として検出", () => {
  ...
  assert.deepEqual(findDuplicates(ds.recommendations), [{ key: "m1|match_1x2|home", ids: ["rec-r1", "rec-r1-dup"] }]);
});
...
test("別系統に同じ試合・市場・選択があっても重複ではない（系統は独立）", () => {
  const ds = makeDataset();
  // rec-r1 と v1-a はどちらも m1 / match_1x2 / home
  assert.deepEqual(findDuplicates(ds.recommendations), []);
